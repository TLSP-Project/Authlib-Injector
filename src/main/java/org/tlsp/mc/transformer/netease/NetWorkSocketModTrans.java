...
public class NetWorkSocketModTrans implements ClassFileTransformer {
    @Override
    public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined, ProtectionDomain protectionDomain, byte[] classfileBuffer) throws IllegalClassFormatException {
        switch (className){
            case "com/netease/mc/mod/network/socket/NetworkSocketMod":
                return doAntiMod(loader, className, classBeingRedefined, protectionDomain, classfileBuffer);
            case "com/netease/mc/mod/network/socket/NetworkClientProxy":
                return doAntiMod(loader, className, classBeingRedefined, protectionDomain, classfileBuffer);
            default:
        ...
        cr.accept(new ClassVisitor(Opcodes.ASM5,cw) {
            ...
            public MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
                ...
                if ("preInit".equalsIgnoreCase(name)){
                    ...
                    mv.visitEnd();
                    return null;
                }else if ("onServerStarting".equalsIgnoreCase(name)){
                    mv.visitInsn(RETURN);
                    mv.visitMaxs(-1, -1);
                    mv.visitEnd();
                    return null;
